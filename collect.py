...
def collect(days_back=None):
    ...
    jobs = {
        "kospi": lambda: fetch_domestic_index(kis, CODES["domestic_index"]["kospi"], since("kospi"), today),
        "kosdaq": lambda: fetch_domestic_index(kis, CODES["domestic_index"]["kosdaq"], since("kosdaq"), today),
        "samsung_elec": lambda: fetch_domestic_stock(kis, CODES["domestic_stock"]["samsung_elec"], since("samsung_elec"), today),
        "sk_hynix": lambda: fetch_domestic_stock(kis, CODES["domestic_stock"]["sk_hynix"], since("sk_hynix"), today),
        "usdkrw": lambda: fetch_overseas(kis, CODES["overseas"]["usdkrw"], since("usdkrw"), today),
        "usdjpy": lambda: fetch_overseas(kis, CODES["overseas"]["usdjpy"], since("usdjpy"), today),
        "ust10y_weekly": lambda: fetch_ust10y(since("ust10y_weekly"), today, kis),
        "wti": lambda: fetch_wti(kis, since("wti"), today),
        "ktb3y": lambda: fetch_ktb3y(since("ktb3y"), today),
        "investor_flow": lambda: fetch_investor(kis, since("investor_flow"), today),
    }
